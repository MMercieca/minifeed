class MiniFeed < ApplicationRecord
  ...
  def ensure_feed_image
    if !self.image.attached?
      ...
      kit = IMGKit.new("<html charset='utf-8' style='max-width: 400px; overflow: none;'>
                          <body style='max-width: 400px; overflow: none'>
                            ...
                            <h1 style='color: white; font-family: sans-serif; width: 400px; font-size: 3em; position: absolute; top: 30%; left: 0px; text-align: center; display: block'>#{self.name}</h1>
                          </body>
                       </html>", width: 400)
      img = kit.to_img(:png)
